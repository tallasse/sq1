...
/*************************************************************************/
  //delete arrays
	...
	if (m_bStartedThisFrame)
		delete [] m_bStartedThisFrame;


	m_lpBuffer = 0;
	m_nCopyCount = 0;
	m_bStartedThisFrame = 0;
...
{
	//DEBUGPRINTF("CSoundManager::play - beginning to play sound at index %d\n", index);

	if(!m_bOperational)return; //bail if not initialized
	//DEBUGPRINTF("CSoundManager::play - looks initialized...\n");

	if(index < 0 || index >= m_nCount)return; //bail if bad index
	//DEBUGPRINTF("CSoundManager::play - index looks good...\n");

	if(m_bStartedThisFrame[index])return; //bail if already playing
	//DEBUGPRINTF("CSoundManager::play - not already playing...\n");

	int copy = 0; //current copy
	DWORD status; //status of that copy

	//DEBUGPRINTF("CSoundManager::play - getting status of first copy at index: %d, copy: %d...\n", index, copy);
	//get status of first copy
	if(FAILED(m_lpBuffer[index][copy]->GetStatus(&status)))
	{
		//DEBUGPRINTF("CSoundManager::play - thingy failed\n");
		status = DSBSTATUS_PLAYING; //assume playing if failed
	}
	else
	{
		//DEBUGPRINTF("CSoundManager::play - thingy didn't fail\n");
	}
	//DEBUGPRINTF("CSoundManager::play - about to start while loop...\n");
	//find next unplayed copy, if any
	while(copy < m_nCopyCount[index] && (status&DSBSTATUS_PLAYING)) //while current copy in use
	{
		//DEBUGPRINTF("CSoundManager::play - looping through while\n");
		if(++copy < m_nCopyCount[index]) //go to next copy
		{
			//DEBUGPRINTF("CSoundManager::play - going to next copy\n");
			if(FAILED(m_lpBuffer[index][copy]->GetStatus(&status)))
			{
				status = DSBSTATUS_PLAYING; //assume playing if failed
				//DEBUGPRINTF("CSoundManager::play - all copies playing\n");
			} // end if failed
		} // end if ++copy > copycount
	} // end while

	//DEBUGPRINTF("CSoundManager::play - unplayed copy found\n");

	//play copy
	if(copy < m_nCopyCount[index]) //if unused copy found
	{
		m_lpBuffer[index][copy]->Play(0, 0, looping?DSBPLAY_LOOPING:0); //play it
		//DEBUGPRINTF("CSoundManager::played sound\n");
	    m_bStartedThisFrame[index] = TRUE; //record fact that we started it
	} // end if unused copy found
}
...
int CSoundManager::LoadSound(char *filename, BYTE **buffer){
  ...
  *buffer = new BYTE[size];
  ...
  {
	  mmioClose(hmfr, 0); 
	  DEBUGPRINTF("CSoundManager::LoadSound - out of memory!\n");
...
{
	if(g_xmlSettings) // if got "settings" tag
	{

		//get "sound" tag
		TiXmlElement* sndSettings = g_xmlSettings->FirstChildElement("sound"); //sound tag
		//get settings
...
void CSoundManager::LoadSounds(int level)
{
  ...
  if(g_xmlSettings){ //got "settings" tag
    ...
    if(found){
      ...
      char buffer[64]; //buffer for file name
      for(s = snd->FirstChildElement("sound"); s; s = s->NextSiblingElement("sound")){
        strncpy(buffer, s->Attribute("file"), sizeof(buffer)); //get file name
        s->Attribute("copies", &copies); //get number of copies
        load(buffer, copies); //load the file //next <sound> tag
      }
    }
  }
  //DEBUGPRINTF("CSoundManager::LoadSounds - m_nMaxSounds: %d\n", m_nMaxSounds);
}
