...
CDebugManager::CDebugManager(){ //constructor
  ...
  //default settings
  strncpy(m_szDebugFileName, "debug.txt", DEBUG_FNAME_SIZE); //file name
  strncpy(m_szDebugIPAddress, "127.0.0.1", sizeof(m_szDebugIPAddress)); //localhost
...
CDebugManager::~CDebugManager(){ //destructor
  ...
  if(m_bHeader){
    ...
    printf("**** Exiting Ned's Turkey Farm\n\n");
...
void CDebugManager::open(){
  //open output file
  if(m_bOutputToFile)
    m_fileHandle=fopen(m_szDebugFileName,"wt"); //open output file
  ...
  if(m_bHeader){
    BOOL temp = m_bPrependFileInfo;
    m_bPrependFileInfo = FALSE;
    printf("**** Starting Ned's Turkey Farm\n");
    m_bPrependFileInfo = temp;
  }
}
...
void CDebugManager::printf(const char* format,...){ //debug printf
  ...
  if(m_bPrependFileInfo)
    sprintf(m_szOutBuffer,"%s(%d): ",m_szFileName,m_nLineNumber);
  else m_szOutBuffer[0] = '\0';

  //append debug message from parameter list
  va_list arglist;
  va_start(arglist,format);
  _vsnprintf((char*)(m_szOutBuffer+strlen(m_szOutBuffer)),
...
void CDebugManager::GetDebugSettings(TiXmlElement* settings){
  ...
  if(ist){
    ...
    if(tag){
      ...
      strncpy(m_szDebugFileName, tag->Attribute("name"), sizeof(m_szDebugFileName));
    }
    ...
    if(tag){
      ...
      strncpy(m_szDebugIPAddress, tag->Attribute("address"), sizeof(m_szDebugIPAddress));
