...
ninjaObj::ninjaObj(D3DXVECTOR3 p, int HP, weaponType* theWeaponList) // constructor
	:enemyObj("ninja", ninja_stand, p, HP, theWeaponList, ENEMY_FACTION)
{
	...
	//DEBUGPRINTF("ninjaObj::constructor - weaponlist is 0: %d, 1: %d, current is: %d\n", weaponList[0], weaponList[1], currWeapon);
}

int ninjaObj::damageAmount()
{
	return g_cRandom.number(10, 30);
}

void ninjaObj::doAction(actionType theAction)
{

}

// run AI thing
void ninjaObj::runAI(gameObj** objectList, int maxObjects)
{
			...
			for (int i = 0; i < maxObjects; i++)
			{
				...
				if (target != NULL) // make sure we're not looking at an empty space
				{
					if (target->faction() == PLAYER_FACTION) // if we're looking at a player
					{
						...
						{
							int decideToRun = g_cRandom.number(0, 1);
							switch(decideToRun)
							{
								case 0:
...
void ninjaObj::hurt(int damage)
{
	if (!isDead())
	{
		...
		else
		{
			...
			if (g_cRandom.number(0, 25) == 0)
			{
				g_cObjectManager.create(OBSTACLE_TYPE, "katana", newLocation);
			}
...
void ninjaObj::doAttack(gameObj* target)
{
	int violence;
...
void ninjaObj::doEscape(gameObj* target)
{
	running = true;
...
} // end ninjaObj::doEscape

void ninjaObj::doWait()
{
	xSpeed = 0;
	zSpeed = 0;
