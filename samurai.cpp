...
samuraiObj::samuraiObj(D3DXVECTOR3 p, int HP, weaponType* theWeaponList) // constructor
	:enemyObj("samurai", sam_stand, p, HP, theWeaponList, ENEMY_FACTION)
{
	...
	//DEBUGPRINTF("samuraiObj::constructor - weaponlist is 0: %d, 1: %d, current is: %d\n", weaponList[0], weaponList[1], currWeapon);
}

int samuraiObj::damageAmount()
{
	return g_cRandom.number(10, 20);
}

void samuraiObj::doAction(actionType theAction)
{

}

// run AI thing
void samuraiObj::runAI(gameObj** objectList, int maxObjects)
{
	active = true;
		...
		if (g_cTimer.elapsed(lastAITime, AIDelayTime)) // if enough time has passed, run AI again
		{
			...
			for (int i = 0; i < maxObjects; i++)
			{
				...
				if (target != NULL) // make sure we're not looking at an empty space
				{
					...
					{
						if (hitPoints < 25) // if we're going to die soon...
						{
...
void samuraiObj::stop()
{
	// something goes here
}

void samuraiObj::move()
{
...
void samuraiObj::hurt(int damage)
{
	if (!isDead())
	{
		int i; 
		...
		else
		{
			...
			if (g_cRandom.number(0, 20) == 0)
			{
				g_cObjectManager.create(OBSTACLE_TYPE, "armor", newLocation);
			}
			else if (g_cRandom.number(0, 10) == 0)
			{
				g_cObjectManager.create(OBSTACLE_TYPE, "bigrice", newLocation);
			}
			else if (g_cRandom.number(0, 5) == 0)
			{
				g_cObjectManager.create(OBSTACLE_TYPE, "smallrice", newLocation);
			}
...
void samuraiObj::doAttack(gameObj* target)
{
	int violence;
...
void samuraiObj::doEscape(gameObj* target)
{
	running = true;
...
} // end samuraiObj::doEscape

void samuraiObj::doWait()
{
	xSpeed = 0;
	zSpeed = 0;
